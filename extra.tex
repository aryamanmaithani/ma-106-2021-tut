...
\begin{enumerate}[leftmargin=*]
	...
	Show that there exists a $7 \times 4$ matrix $X \neq O$ such that $AXB = O.$
	%
...
\begin{enumerate}[leftmargin=*]
	...
	Using the above exercise, it would be easy to conclude that if $AB = I$ \emph{and} $A$ has a left inverse, then $BA = I.$ However, we are still left to show that $A$ actually does have a left inverse.\\
...
\begin{enumerate}[leftmargin=*]
	...
	Let $S$ be a (finite) linearly independent subset of $V$ and let $T:V\to W$ be a linear map.\\
	Show that if $T$ is injective (one-to-one), then $T(S)$ is a linearly independent subset of $W.$
	%
	\item Let $V$ and $W$ be vector spaces over some field $\mathbb{F}.$\\
	Let $S$ be a (finite) spanning subset of $V$ and let $T:V\to W$ be a linear map.\\
...
\begin{enumerate}[leftmargin=*]
	...
	%
	\item Let $A \in M_n(\mathbb{C}).$ \\
	Show that if $A$ is unitary, then $\lvert\det A\rvert = 1$. \\
	Show that the converse is not true.
	%
...
\begin{enumerate}[leftmargin=*]
	...
	Show that $(G_1, +)$ and $(G_2, +)$ are isomorphic as groups if and only if $(G_1, +, \cdot)$ and $(G_2, +, \cdot)$ are isomorphic as vector spaces (over $\mathbb{Q}$).
	%
	\item (Abusing notation now and not writing the operations.) \\
	Let $G_1$ and $G_2$ be vector spaces over some field $\mathbb{F}.$\\
	Show that $G_1$ and $G_2$ are isomorphic as additive groups if they are isomorphic as vector spaces over $\mathbb{F}.$\\
	Show that the converse need not be true.\\
	(The previous question shows that the converse does hold in the special case that $\mathbb{F} = \mathbb{Q}.$ For what other $\mathbb{F}$ does it hold?)
	%
